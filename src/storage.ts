...
export function normalize(input: unknown): AppData {
  ...
  const barterSession = { remainingNegotiation: clamp(Number(rawBarter?.remainingNegotiation)), costPerExchange: clamp(Number(rawBarter?.costPerExchange)), addToInventory: rawBarter?.addToInventory !== false, exchangeCounts: Object.fromEntries(Object.entries(rawBarter?.exchangeCounts || {}).map(([id, count]) => [id, clamp(Number(count))])) }

...
export function useDriveSync(data: AppData, replaceLocal: (data: AppData) => void) {
  ...
  const authorize = useCallback(async (interactive: boolean) => { const clientId = getClientId(); if (!clientId) { setStatus('failed'); setError('Google OAuth Client ID를 설정하세요.'); return } try { setStatus('connecting'); await loadGoogleIdentity(); const client = window.google!.accounts.oauth2.initTokenClient({ client_id: clientId, scope: 'https://www.googleapis.com/auth/drive.appdata', callback: response => { if (!response.access_token) { setStatus(interactive ? 'failed' : 'disconnected'); setError(interactive ? (response.error_description || 'Google 인증에 실패했습니다.') : 'Drive 세션을 조용히 복구하지 못했습니다. 연결 버튼으로 다시 인증할 수 있습니다.'); return } token.current = response.access_token; setDriveConnected(true); compare().catch(e => { setStatus('failed'); setError(e instanceof Error ? e.message : 'Drive 동기화에 실패했습니다.') }) }, error_callback: issue => { setStatus(interactive ? 'failed' : 'disconnected'); setError(interactive ? (issue.message || 'Google 인증 창을 닫았습니다.') : 'Drive 세션을 조용히 복구하지 못했습니다. 연결 버튼으로 다시 인증할 수 있습니다.') } }); client.requestAccessToken({ prompt: interactive ? 'select_account' : 'none' }) } catch (e) { setStatus(interactive ? 'failed' : 'disconnected'); setError(e instanceof Error ? e.message : 'Google Identity Services 오류') } }, [compare])
  // Google 로그인·권한 쿠키가 살아 있으면 화면 없이 새 단기 토큰을 받는다. prompt: 'none'은
  // 계정 선택/동의 화면을 절대 표시하지 않으므로, 자동 복구가 팝업으로 바뀌지 않는다.
  useEffect(() => { if (isDriveConnected()) void authorize(false) }, [authorize])

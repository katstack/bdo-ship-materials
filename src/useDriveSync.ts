...
import { getClientId, getDriveFile, loadGoogleIdentity, saveDriveFile, setDriveConnected } from './drive'
...
export function useDriveSync(data: AppData, replaceLocal: (data: AppData) => void) {
  // GIS의 prompt: ''도 계정 선택 창을 띄울 수 있다. 새로고침은 절대 인증을 시작하지 않고,
  // 사용자가 연결 버튼을 누른 현재 세션에서만 Drive를 사용한다.
